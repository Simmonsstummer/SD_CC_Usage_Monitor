# CC Usage Monitor — Stream Deck Plugin

Monitor your **Claude Code (Anthropic) session usage** in real time, directly on an Elgato Stream Deck key.

The key shows the **5-hour session percentage** as a large number (green / yellow / red), with a countdown to the next reset below it. It also signals extra usage and rate-limited states.

---

## Install

Find **CC Usage Monitor** in the [Elgato Marketplace](https://marketplace.elgato.com) and click Install, or search for it directly inside the Stream Deck software.

**Requirements:** Stream Deck software 6.4+, Claude Code installed and signed in on Windows.

---

- [Documentation & screenshots](marketplace/README.md)
- [Privacy Policy](marketplace/PRIVACY.md)
- [Support & FAQ](marketplace/SUPPORT.md)
- [Report an issue](https://github.com/Simmonsstummer/SD_CC_Usage_Monitor/issues)
